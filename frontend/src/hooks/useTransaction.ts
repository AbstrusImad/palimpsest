...
function friendly(e: unknown): string {
  ...
  if (/LackOfFundForMaxFee|insufficient funds/i.test(msg)) {
    return 'Your wallet is below the fee reserve for AI transactions (mostly refunded). Claim test GEN at testnet-faucet.genlayer.foundation';
  }
  if (/reject|denied|4001/i.test(msg)) return 'You set down the quill';
  if (/rate limit|429|too many/i.test(msg)) {
    return 'The network is congested, your entry is still being weighed';
  }
  ...
  return 'The scribing failed. Please try again.';
}
    ...
    try {
      ...
      if (status === 'ACCEPTED' || status === 'FINALIZED') {
        ...
        setState((s) => ({
          ...
          phase: 'error',
          liveStatus: status,
          error: 'The Loremaster and validators could not agree. Please scribe again.',
        }));
        ...
        setState((s) => ({
          ...
          phase: 'error',
          liveStatus: status,
          error: 'The network is still weighing your entry. Check the explorer for the ruling.',
        }));
      }
    } catch (e) {
